...
class PromptLateTailRpc implements StdioProcess {
  private messageHandlers: Array<(m: JsonRpcMessage) => void> = [];
  private timers: NodeJS.Timeout[] = [];
...
test('AcpClient prompt drain captures delayed tail updates after prompt result', async () => {
  const db = createDb();
  const updates: string[] = [];
  const rpc = new PromptLateTailRpc();
  createSession(db, {
    sessionKey: 'session-late-tail',
    agentCommand: 'node',
    agentArgs: [],
    cwd: '/tmp',
    loadSupported: false,
  ...
  const client = new AcpClient({
    ...
    agentArgs: [],
    rpc,
    promptUpdateIdleMs: 250,
    promptUpdateMaxWaitMs: 800,
    events: {
      onSessionUpdate: (_run, _sessionId, update) => {
        updates.push(String(update?.content?.text ?? ''));
      },
  ...
  const elapsedMs = Date.now() - startedAt;
  assert.equal(result.stopReason, 'end');
  assert.deepEqual(updates, ['tail-chunk']);
  assert.ok(
    elapsedMs >= 350 && elapsedMs < 650,
    `unexpected drain time for delayed tail: ${elapsedMs}ms`,
  );
